...
/* PM-Pro nonce source hook (set by the app; see hn_compat.h) and the per-nonce
 * callback the device collection drives. */
int (*hardnested_pmpro_collect)(uint8_t, uint8_t, const uint8_t *, uint8_t,
                                uint8_t, hn_feed_fn, uint32_t *) = NULL;
volatile uint64_t hardnested_recovered_key = 0;
volatile int hardnested_recovered = 0;
static int add_nonce(uint32_t nonce_enc, uint8_t par_enc); /* fwd decl */
static void hn_add_nonce_cb(uint32_t nonce_enc, uint8_t par_enc) {
    num_acquired_nonces += add_nonce(nonce_enc, par_enc);
}
...
static int acquire_nonces(uint8_t blockNo, uint8_t keyType, uint8_t *key, uint8_t trgBlockNo, uint8_t trgKeyType) {
    ...
    /* PM-Pro nonce source replaces libnfc collection: cmd 0x30 collects a batch
     * of encrypted nonces; each is fed through add_nonce() via the hook. */
    if (!hardnested_pmpro_collect)
        return 4;
    do {
        uint32_t cu = 0;
        int got = hardnested_pmpro_collect(blockNo, keyType, key, trgBlockNo,
                                           trgKeyType, &hn_add_nonce_cb, &cu);
        cuid = cu;
        if (got <= 0)
            return num_acquired_nonces ? 0 : 5;   /* device gave no nonces */
        if (first_byte_num == 256) {
            if (hardnested_stage == CHECK_1ST_BYTES) {
                for (uint16_t i = 0; i < NUM_SUMS; i++) {
...
int mfnestedhard(uint8_t blockNo, uint8_t keyType, uint8_t *key, uint8_t trgBlockNo, uint8_t trgKeyType, bool hard_low_memory) {
    ...
    char progress_text[80];
    /* cuid is set by the PM-Pro nonce collection inside acquire_nonces() */
